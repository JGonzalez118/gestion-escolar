...
- Routing in `src/App.jsx`: the `Sidebar` renders on every page except `/login`. Pages: `Dashboard` (`/`), `Salon` (`/salon`), `Estudiantes` (`/estudiantes`), `Actividades` (`/actividades`, lists activities per day and lets a docente grade students), `Grados` (`/grados`, docente-only), `Boletin` (`/boletin`, docente-only report card). Reusable pieces in `src/components/`: `Sidebar`, `Navbar`, `PrivateRoute`, `AsistenciaCard`, `ClaseActualCard`, `CrearActividadForm`.
- Page-specific styles live in `src/styles/` (e.g. `boletin.css`, `actividades.css`, `estudiantes.css`, `salon.css`, `login.css`).
